	...
	factory: (require) => {
		...
		*/
		function ModelCapabilities(props) {
			...
			const providers = snapshot.status === "ready" ? providersOf$1(snapshot) : {};
		...
		function apply(ctx) {
			...
			ctx.slots.inject("settings.plugin.item", function* () {
				yield ctx.slots.register({
					...
					inject: () => {
						return {
							scope: ctx.settingsScope.bind({ namespace: THINKING_LEVELS_NS }),
							piAiScope: ctx.settingsScope.bind({ namespace: "llm-pi-ai" })
						};
					}
				}, ThinkingLevelsCard);
			});
			...
			ctx.slots.inject("conversation.input.right", function* () {
				yield ctx.slots.register({
					...
					inject: () => {
						return {
							piAiScope: ctx.settingsScope.bind({ namespace: "llm-pi-ai" }),
							deepseekScope: ctx.settingsScope.bind({ namespace: "llm-deepseek" })
						};
					}
				}, ContextQuick);
			});
