...
export const zh = {
  ...
  'card.capabilities.empty': '短路名单（llm-openai-completions）中没有 provider，或名单内 provider 未在 llm-pi-ai 配置模型。请先在「设置 → 模型」添加提供方与模型，并把要编辑的自定义网关加入短路名单。',
  ...
  'card.capabilities.takeover': '短路接管',
  'card.capabilities.notTakenOver': '未启用短路接管：该 provider 由 pi-ai 原生服务。开启「短路接管」后才能编辑此处的思考/effort 配置。',
...
export const en: Record<keyof typeof zh, string> = {
  ...
  'card.capabilities.empty': 'No provider is in the takeover list (llm-openai-completions), or the listed providers have no models configured in llm-pi-ai. Add a provider and models under Settings → Models first, and put the custom gateway you want to edit into the takeover list.',
  ...
  'card.capabilities.takeover': 'Short-circuit takeover',
  'card.capabilities.notTakenOver': 'Takeover is off: this provider is served by pi-ai natively. Enable "Short-circuit takeover" to edit its thinking/effort configuration here.',
...
export const ja: Record<keyof typeof zh, string> = {
  ...
  'card.capabilities.empty': '引き継ぎリスト（llm-openai-completions）にプロバイダーがないか、リスト内プロバイダーに llm-pi-ai のモデル設定がありません。先に「設定 → モデル」でプロバイダーとモデルを追加し、編集したいカスタムゲートウェイを引き継ぎリストに入れてください。',
  ...
  'card.capabilities.takeover': 'ショートサーキット引き継ぎ',
  'card.capabilities.notTakenOver': '引き継ぎが無効です：このプロバイダーは pi-ai がネイティブに提供します。「ショートサーキット引き継ぎ」を有効にしてから、ここで思考/effort 設定を編集してください。',
...
export const ko: Record<keyof typeof zh, string> = {
  ...
  'card.capabilities.empty': '인계 목록(llm-openai-completions)에 제공자가 없거나, 목록 내 제공자에 llm-pi-ai 모델 설정이 없습니다. 먼저 「설정 → 모델」에서 제공자와 모델을 추가하고, 편집할 사용자 지정 게이트웨이를 인계 목록에 넣으세요.',
  ...
  'card.capabilities.takeover': '단락 인계',
  'card.capabilities.notTakenOver': '인계가 꺼져 있습니다: 이 제공자는 pi-ai가 기본 제공합니다. 「단락 인계」를 켠 후 여기서 사고/effort 구성을 편집하세요.',
