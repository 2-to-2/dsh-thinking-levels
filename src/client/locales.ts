...
export const zh = {
  ...
  'card.level.on': 'on — 开启思考（映射到模型默认强度，如 high）',
...
export const en: Record<keyof typeof zh, string> = {
  ...
  'card.level.on': 'on — enable thinking (maps to the model’s default strength, e.g. high)',
...
export const ja: Record<keyof typeof zh, string> = {
  ...
  'card.level.on': 'on — 思考を有効化（モデルの既定強度（例：high）にマップ）',
  ...
  'card.capabilities.empty': 'llm-pi-ai にカスタムモデルが設定されていません。「設定 → モデル」でプロバイダーとモデルを追加してください。',
...
export const ko: Record<keyof typeof zh, string> = {
  ...
  'card.level.on': 'on — 사고 활성화(모델 기본 강도(예: high)로 매핑)',
  ...
  'card.capabilities.empty': 'llm-pi-ai에 사용자 지정 모델이 없습니다. 먼저 「설정 → 모델」에서 제공자와 모델을 추가하세요.',
