...
| `on` | モデルの既定強度（例：`high`）で思考有効化。トグルのみのモデルでは On/Off スイッチ | モデルセレクター / 既定レベル |
...
ワイヤーレベルの事実（公式 DeepSeek ドキュメントと dsh の `llm-deepseek` アダプターで確認）：deepseek-v4-flash / v4-pro では `low` が 1:1 で有効、`medium` / `xhigh` は `high` に畳み込まれます。アダプターは `off | low | high | max` のみ受け付け、それ以外は `UNSUPPORTED_REASONING_EFFORT` で拒否します——`auto` はプラグインのマスク層で、API には送信されず、注入前に必ず具体的なワイヤーレベルへ解決されます。
...
| `off` / `on` / `minimal` / `low` / `medium` / `high` / `xhigh` / `max` | **手動選択が優先**——プラグインは介入しません（`on` はモデルの既定強度へクランプ） |
