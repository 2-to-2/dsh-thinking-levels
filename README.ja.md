...
> **バージョン互換（2.0.0-beta.3+）：** DSH ≥ 0.1.2-alpha.1 に対応。DSH 0.1.5 でプラグイン設定カードのスロットが `settings.plugin.item` から `settings.plugins.tab` に変更されましたが、本プラグインはホストが宣言したスロットへ自動的に登録するため、両バージョン線でカードが表示されます。既存プロファイルを DSH 0.1.5 にアップグレードした後にカードが消えた場合は、まずブラウザーを強制リロードしてください（client combo キャッシュの陳腐化は 0.1.5 の既知のアップグレード問題です）。
