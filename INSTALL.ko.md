...
DSH를 재시작하고 웹 페이지를 새로고침하세요. 「설정 → 플러그인 → 설정 가능한 플러그인」을 열고 「사고 수준」 카드를 펼치세요.

1. 수준 선택기는 8개 표준 수준에 `auto`를 더한 목록을 제공합니다. 아래에 스케줄러 토글이 있습니다.
2. 모델 기능 섹션은 제공자별로 그룹화됩니다. 각 모델 행은 텍스트/이미지 배지와 선언된 컨텍스트 길이를 표시하며, 펼치면 수준별 편집기가 됩니다.
3. 수준을 체크하고 게이트웨이 전송 값(예: `high` → `ultra`)을 입력한 뒤 「수준 적용」을 누르세요. `off`를 비워 두면 전송되지 않습니다.
4. 검색 상자로 모델을 필터링하고, 공식/일반 프리셋으로 모든 사고 모델에 일괄 적용할 수 있습니다.
...
| 설정 카드 쓰기 실패 | llm-pi-ai 스키마에 거부됨(예: 사고 수준이 없는 `reasoningEfforts` 테이블). 입력 값 확인. |
